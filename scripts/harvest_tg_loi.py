...
def fetch_open_html(url: str) -> tuple[str, list[str]]:
    if not url:
        return "", []
    try:
        ...
        try:
            for df in pd.read_html(r.text):
                ...
                blob = cols + " " + df.astype(str).head(40).to_csv(index=False)
                if LOI_TERMS.search(blob) or TG_TERMS.search(blob):
                    tables.append(blob[:5000])
        except Exception:
            pass
        return text, tables[:6]
    except requests.RequestException:
        return "", []
...
def main() -> None:
    ...
    for key, w in discovered.items():
        ...
        if not fulltext:
            ...
            fulltext, tables = fetch_open_html(oa_url)
            fulltext_url = oa_url if fulltext else ""
        combined = fulltext if fulltext else abstract
